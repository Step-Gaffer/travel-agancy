<div class="content-card">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2><i class="fas fa-plane-departure me-2"></i>Редактирование поездки</h2>
        <a href="index.php?entity=trip&action=show&id=<?= $trip['TripID'] ?>" class="btn btn-outline-secondary">
            <i class="fas fa-times me-1"></i> Отмена
        </a>
    </div>

    <form method="POST" action="index.php?entity=trip&action=edit&id=<?= $trip['TripID'] ?>">
        <div class="card mb-4">
                ...
                <div class="mb-3">
                    ...
                    <select name="TourID" class="form-select" required>
                        <?php foreach ($tours as $tour): ?>
                            <option value="<?= $tour['TourID'] ?>" <?= ($trip['TourID'] == $tour['TourID']) ? 'selected' : '' ?>>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="date" name="DepartureDate" class="form-control"
                            value="<?= $trip['DepartureDate'] ?>" required>
                    </div>

                    <div class="col-md-6 mb-3">
                        <label class="form-label fw-bold">Номер рейса <span class="text-danger">*</span></label>
                        <input type="text" name="FlightNumber" class="form-control" value="<?= $trip['FlightNumber'] ?>"
                            required>
                    </div>
                </div>
            </div>
